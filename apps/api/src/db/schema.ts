...
import { bigint, date, decimal, index, integer, jsonb, pgTable, text, timestamp, uniqueIndex, uuid } from "drizzle-orm/pg-core";
...
export const stockPriceHistory = pgTable(
  ...
  (table) => [index("price_history_stock_time_idx").on(table.stockId, table.recordedAt)]
);
...
export const transaction = pgTable(
  "transaction",
  {
    ...
    transactionType: text("transaction_type", { enum: ["buy", "sell", "deposit", "withdrawal"] }).notNull(),
...
export const stockPriceHistoryRelations = relations(stockPriceHistory, ({ one }) => ({
  stock: one(stock, { fields: [stockPriceHistory.stockId], references: [stock.id] }),
}));
